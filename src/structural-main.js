...
import { analyzeStructuralProbability } from './structural-probability.js'
  ...
  <main class="struct-shell">
    <header class="struct-hero">
      <div>
        ...
        <p>Missing • Frequency • Transition • Mirror • Balance • Walk-forward</p>
      </div>
    ...
    <section id="result" class="struct-result hidden" aria-live="polite">
      ...
      <article class="struct-card">
        <div class="struct-card-head"><small>🎯 เจาะ 3 บน</small><span id="patternLabel">Pattern: —</span></div>
      ...
      <p class="struct-note">Structural Score ใช้จัดอันดับเชิงสถิติ ไม่ใช่เปอร์เซ็นต์โอกาสถูกรางวัล และเลขหายเป็นเพียง Candidate Signal</p>
    </section>
  </main>
`
...
function patternText(analysis) {
  if (!analysis.patternForecast) {
    return `หลักฐานยังน้อย • ${analysis.transition.samples} sample`
  }
  return `${analysis.patternForecast.category} • ${analysis.patternForecast.percentage}% • ${analysis.transition.samples} sample`
}

function renderEvidence(analysis) {
  const current = analysis.currentPattern
  els.evidence.innerHTML = `
    <p><b>ล่าสุด</b><span>${current.category}</span></p>
...
function metric(label, value, samples) {
  return `<p><b>${label}</b><span>${samples ? `${value}%` : '—'}${samples ? ` • ${samples} งวด` : ''}</span></p>`
}
...
function renderResult(marketName, analysis) {
  ...
  els.patternLabel.textContent = `Pattern: ${patternText(analysis)}`
...
async function calculate() {
  ...
  setStatus('กำลังคำนวณ Structural Probability...', 'loading')

  try {
    const history = await loadRecentResults(marketKey, 30)
    const analysis = analyzeStructuralProbability(history, { includeBacktest: true, maxBacktest: 10 })
    renderResult(selected?.market_name || marketKey, analysis)
    setStatus(`พร้อม • ใช้ข้อมูล ${analysis.sampleSize} งวด • ไม่มี Random`, 'ready')
  } catch (error) {
    console.error(error)
