...
import { loadLatestThreeResults } from './flow-data.js'
import { loadMarkets } from './supabase.js'

const app = document.querySelector('#app')

app.innerHTML = `
  <main class="shell">
    <div class="ambient-lights" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div>
    ...
    <section id="controlCard" class="control-card">
      ...
      <div id="marketPicker" class="market-picker">
        ...
        <div class="search-field">
          ...
          <input id="marketSearch" type="search" inputmode="search" autocomplete="off" placeholder="พิมพ์ชื่อตลาด เช่น ลาว Extra" aria-controls="searchResults" aria-expanded="false" disabled>
        ...
        <div class="select-row">
          <div class="select-wrap">
            <select id="market" aria-label="เลือกตลาด" disabled><option>กำลังโหลดตลาด...</option></select>
          </div>
          <button id="refresh" class="icon-button" type="button" aria-label="คำนวณใหม่" title="คำนวณใหม่">↻</button>
        </div>
      </div>
      <div id="status" class="status loading"><span></span>กำลังเชื่อมต่อข้อมูล</div>
    </section>
...
function pairItems(items, valueKey, label) {
...
function renderResult(marketName, analysis) {
  ...
  els.rudNumbers.innerHTML = analysis.rud.map((digit, index) => `
    <div class="flow-rud ${index === 0 ? 'main' : 'secondary'}">
      <b>${digit}</b>
      <small>${index === 0 ? 'รูดหลัก' : 'รูดรอง'}</small>
    </div>
  `).join('')
...
async function initialize() {
  try {
    ...
    els.market.disabled = false
    setStatus(`พร้อมใช้งาน • ${markets.length} ตลาด • FLOW CORE`, 'ready')
...
els.copy.addEventListener('click', async () => {
  if (!current) return
  const text = formatFlowCopyText(current)
  try {
